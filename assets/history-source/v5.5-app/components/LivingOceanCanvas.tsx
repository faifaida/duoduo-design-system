...
export function LivingOceanCanvas({ calm = false }: { calm?: boolean }) {
  ...
  useEffect(() => {
    ...
    const drawDistortedOcean = (width: number, height: number, time: number) => {
      ...
      const slice = calm ? 14 : 10;
      const amplitude = calm ? 2.5 : 5.5;
      context.save();
      context.globalAlpha = calm ? 0.2 : 0.34;
      ...
      for (let y = 0; y < height; y += slice) {
        ...
        const pointerPull = pointer.active
          ? Math.exp(-Math.pow((y - pointer.y) / 150, 2)) * Math.sin(time * 2.4 + y * 0.035) * 4
          : 0;
        const sx = sourceX;
    ...
    const drawCaustics = (width: number, height: number, time: number) => {
      ...
      for (let band = 0; band < bands; band += 1) {
        ...
        context.strokeStyle = `rgba(226,255,249,${0.026 + (band % 3) * 0.012})`;
        context.lineWidth = band % 4 === 0 ? 1.5 : 0.7;
        context.stroke();
      }

      if (pointer.active && !reduced) {
        const pulse = 34 + (frame % 110) * 0.72;
        for (let ring = 0; ring < 3; ring += 1) {
          context.beginPath();
          context.ellipse(pointer.x, pointer.y, pulse + ring * 26, (pulse + ring * 26) * 0.28, -0.05, 0, Math.PI * 2);
          context.strokeStyle = `rgba(182,255,244,${0.16 - ring * 0.035})`;
          context.lineWidth = 1;
          context.stroke();
        }
      }
  ...
  return <canvas ref={canvasRef} className="living-ocean-canvas" aria-hidden="true" />;
}
