            ...
            <section className="tool-desktop" aria-label="Take something desktop">
              ...
              <div className="tool-desktop-grid">
                {publicTools.map((tool) => (
                  <button type="button" className={`tool-desktop-icon tool-kind-${tool.kind.toLowerCase()}`} key={tool.id} onClick={() => void openTool(tool)}>
                    <span className="tool-file-shape" aria-hidden="true"><i>{tool.symbol}</i><small>.MD</small></span>
                    <b>{tool.title}</b>
                    <em>{tool.zh}</em>
                    <small>{tool.kind} · {tool.number}</small>
                  </button>
                ))}
                <span className="tool-desktop-empty"><i>＋</i><b>SKILLS</b><small>为未来工具预留</small></span>
                <span className="tool-desktop-empty"><i>＋</i><b>PROMPTS</b><small>更多公开提示词</small></span>
                <span className="tool-desktop-empty"><i>＋</i><b>WORKFLOWS</b><small>持续生长的工作流</small></span>
              </div>
              <footer><span>03 OBJECTS ONLINE</span><b>CLICK AN ICON TO OPEN · 点击图标预览</b><i>SPACE RESERVED FOR MORE ↓</i></footer>
