  ...
  return (
    <SceneShell bare current="stories" className="stories-scene stories-v2 stories-v5" id="stories">
      ...
      <section className="trunk-world" aria-label="An open travel trunk filled with stories">
        ...
        <a className="wear-origin-tag paper-texture" href="#duoduo-wear">
...
export function WorkV5() {
  ...
  const openCase = () => {
    ...
    document.getElementById(`case-${project.id}`)?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  ...
  return (
    <SceneShell bare current="work" className="work-scene work-v2 work-v5" id="work">
      ...
      <section className="field-studio-world" aria-label="A field studio beside the sea">
        ...
        <motion.article className="studio-dossier paper-texture" key={`${mode}-${project.id}`} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
          {mode === "selected" && (
            <>
      ...
      <section className="work-case-library" aria-label="Full project cases">
        <header><span>FIELD FILES · 田野档案</span><h2>SELECTED CASES<br /><b>完整案例</b></h2><p>不是脱离场景的作品集页面，而是把结果重新放回问题、行动与仍未完成的现实里。</p></header>
        {workProjects.map((item) => (
          <article id={`case-${item.id}`} className={`full-case paper-texture ${item.id === project.id ? "is-active" : ""}`} key={item.id}>
            <div className="full-case-cover"><img src={item.image} alt="" loading="lazy" /><span>{item.number}</span></div>
            <div className="full-case-copy">
              <StatusStamp tone={item.id === "duoduo-wear" ? "blue" : "teal"}><T entry={item.status} /></StatusStamp>
              <h3><BilingualText entry={item.title} primary="en" /></h3>
              <ol>
                <li><b>01 PROJECT CONTEXT｜项目背景</b><BilingualText entry={item.copy} primary="zh" /></li>
                <li><b>02 THE QUESTION｜我面对的问题</b><BilingualText entry={item.problem} primary="zh" /></li>
                <li><b>03 MY ROLE｜我的角色</b><BilingualText entry={item.role} primary="zh" /></li>
                <li><b>04 WHAT I DID｜我做了什么</b><BilingualText entry={item.actions} primary="zh" /></li>
                <li><b>05 OUTCOME｜实际成果</b><BilingualText entry={item.outcome} primary="zh" /></li>
                <li><b>06 STILL OPEN｜仍未解决</b><BilingualText entry={item.unresolved} primary="zh" /></li>
                <li><b>07 NEXT｜下一步</b><BilingualText entry={item.unresolved} primary="zh" /></li>
              </ol>
...
const seedStars: VisitorStar[] = [
  { id: "light-01", nickname: "M.", city: "Shanghai", message: "在这里停了一会儿。自由和扎根原来真的可以是同一个问题。", date: "2026.07", reply: "谢谢你看见了这条暗线。", x: 18, y: 27 },
  { id: "light-02", nickname: "Lina", city: "Colombo", message: "Your route feels less like a résumé and more like a living tide.", date: "2026.07", x: 43, y: 16 },
  { id: "light-03", nickname: "阿岛", city: "Luoyang", message: "希望有一天能在懒懒岛见到真正从远方回来的人。", date: "2026.07", x: 64, y: 33 },
  { id: "light-04", nickname: "N.", city: "Bali", message: "Keep the roots moving.", date: "2026.07", reply: "I will.", x: 78, y: 21 },
];
    ...
    <SceneShell bare current="about" className="about-scene about-v2 about-v5" id="about">
      ...
      <section ref={starFieldRef} onPointerMove={wakeStars} onClick={(event) => event.target === event.currentTarget && setOpenStar(null)} className="life-voyage-stage">
        ...
        <motion.article className="voyage-story-note paper-texture" key={node.year} initial={{ opacity: 0, x: -14 }} animate={{ opacity: 1, x: 0 }}>
          <span>{node.year} · {text(node.place, "en")}</span>
          <h1>{text(node.title, "en")}{locale === "original" && <b>{node.title.zh}</b>}</h1>
          <div className="voyage-card-photos">
            {node.photos.map((photo) => <img key={photo} src={photo} alt={`${text(node.title)} stage`} loading="lazy" />)}
          </div>
          <dl>
        ...
        <div className="visitor-star-field" aria-label="Visitor constellation">
          {stars.map((star) => (
            <div
              ...
              onMouseEnter={() => setOpenStar(star.id)}
            >
              <button
        ...
        <aside className="guestbook-panel">
          <header><span>VISITOR LIGHTS · 来访星光</span><h2>LEAVE A LIGHT<br /><b>留下一束光</b></h2></header>
          <form onSubmit={submitMessage}>
            <input name="nickname" required maxLength={32} placeholder="Nickname · 昵称" />
            <input name="city" maxLength={48} placeholder="City (optional) · 城市（可选）" />
            <textarea name="message" required minLength={20} maxLength={240} placeholder="20–120 个中文字符，或相应长度的英文内容" />
            <input name="email" type="email" placeholder="Email (private, optional) · 邮箱（不公开）" />
            <input name="website" className="honeypot" tabIndex={-1} autoComplete="off" aria-hidden="true" />
            <button disabled={submitState === "sending"} type="submit">SEND TO MODERATION｜提交审核</button>
          </form>
          ...
          <div className="recent-lights">
            <b>RECENT LIGHTS｜最近留言</b>
            {stars.slice(0, 3).map((star) => <span key={star.id}><strong>{star.nickname}</strong><small>{star.city}</small><p>{star.message}</p></span>)}
          </div>
        </aside>
      </section>
    </SceneShell>
...
const wearProgress = [
  entry("BRAND ORIGIN", "品牌起源"),
  entry("WHY SWIMWEAR", "为什么开始做泳衣"),
  entry("FIRST SAMPLES", "第一批样品"),
  entry("FIT PROBLEMS", "当前版型问题"),
  entry("BODY TESTING", "身体测试过程"),
  entry("SRI LANKA TEST", "斯里兰卡市场测试"),
  entry("WHAT IS CHANGING", "正在修改的部分"),
  entry("NEXT", "品牌下一步"),
] as const;
...
export function SurfV5() {
  ...
  return (
    <SceneShell bare current="surf" className="surf-scene surf-v5" id="surf">
      ...
      <SceneTitle number="04" en="SURF & WEAR" zh={locale === "original" ? "冲浪与穿戴" : ""} subtitle="THE WAVE SCHOOL & COASTAL LAB · 浪上学校与海边实验室" symbol="≋" />

      <section className="surf-hero-copy">
        <SymbolSeal symbol="≋" label={`${content.surf.sealLabel.en} ${content.surf.sealLabel.zh}`} />
        <small>01 THE SEA｜海与我的冲浪生活</small>
      ...
      <div className="surf-break" aria-hidden="true"><span className="surf-line line-one" /><span className="surf-line line-two" /><span className="surf-line line-three" /><i className="surf-board">DUODUO</i></div>

      <motion.section className="surf-log-panel paper-texture" whileHover={{ y: -4 }} transition={{ duration: .35 }}>
        <header><StatusStamp><T entry={content.surf.logStatus} /></StatusStamp><small>02 THE PRACTICE｜练习与身体</small><h2><T entry={content.surf.logTitle} em /></h2></header>
        <div className="surf-log-entries">
          {surfLogs.map(([title, copy], index) => <article key={title.en}><span>0{index + 1}</span><h3><BilingualText entry={title} primary="en" /></h3><p><BilingualText entry={copy} primary="zh" /></p></article>)}
        </div>
      ...
      <motion.section id="duoduo-wear" className="surf-wear-lab paper-texture wear-exhibition" whileHover={{ y: -3 }} transition={{ duration: .35 }}>
        <div className="wear-lab-mark"><img src="/brand/duoduo-symbol.png" alt="" /><span>✦</span></div>
        <div className="wear-lab-copy">
          <small>03 DUODUO WEAR｜品牌展览与项目进展</small>
          <StatusStamp tone="blue">SWIM — PROTOTYPE BATCH 01</StatusStamp>
          <h2>DUODUO WEAR</h2>
          <p><BilingualText entry={content.surf.wearCopy} primary="zh" /></p>
        </div>
        <div className="wear-progress-grid">
          {wearProgress.map((item, index) => <span key={item.en}><i>0{index + 1}</i><BilingualText entry={item} primary="en" /></span>)}
        </div>
        {wearUrl ? (
          <a href={wearUrl} target="_blank" rel="noreferrer">VISIT DUODUO WEAR<small>进入独立网站 ↗</small></a>
        ) : (
          <p className="wear-site-status">IN DEVELOPMENT<small>独立品牌网站正在生长</small></p>
        )}
      </motion.section>
    </SceneShell>
  );
