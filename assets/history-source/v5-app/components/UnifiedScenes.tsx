...
export function AiScene() {
  ...
  const exampleQuestions = [
    "为什么多多把旅行、青旅、泳衣和 AI 放在同一个世界里？",
    "多多现在正在做什么？",
    "我可以如何与多多合作？",
    "Duoduo Wear 目前进行到哪一步？",
  ];
        ...
        <div className="os-content">
          ...
          {tab === "ask" && (
            <section className="ask-panel">
              ...
              <div className="ai-question"><input id="duoduo-ai-question" value={question} disabled={pending} onChange={(event) => setQuestion(event.target.value)} onKeyDown={(event) => event.key === "Enter" && void ask()} placeholder={locale === "original" ? "向多多询问旅行、项目、思考或正在进行的实验…… / Ask about my travels, projects, ideas or unfinished experiments..." : text(content.ai.askPlaceholder)} aria-label={`${content.ai.askPromptLabel.en} · ${content.ai.askPromptLabel.zh}`} /><button type="button" disabled={pending} onClick={() => void ask()}>{pending ? "···" : <BilingualText entry={content.ai.askSend} primary="en" />}</button></div>
              <small>Based on approved public materials only. · 只基于已批准公开的资料回答。</small>
            </section>
          )}
          {tab === "take" && (
            <section className="tool-grid">
              <article><span>01</span><h2>PERSONAL COMPANY INVENTORY<small>个人公司自我盘点 Prompt</small></h2><p><BilingualText entry={content.ai.tool1Copy} primary="zh" /></p><a href="/tools/personal-company-inventory.md" download>OPEN / DOWNLOAD｜打开或下载 ↗</a></article>
              <article><span>02</span><h2>TRAVEL LEARNING NOTE<small>旅行学习记录模板</small></h2><p><BilingualText entry={content.ai.tool2Copy} primary="zh" /></p><a href="/tools/travel-learning-note.md" download>OPEN / DOWNLOAD｜打开或下载 ↗</a></article>
              <article><span>03</span><h2>UNFINISHED EXPERIMENT REVIEW<small>未完成实验复盘卡</small></h2><p><BilingualText entry={content.ai.tool3Copy} primary="zh" /></p><a href="/tools/unfinished-experiment-review.md" download>OPEN / DOWNLOAD｜打开或下载 ↗</a></article>
            </section>
          )}
          {tab === "now" && (
          ...
          {tab === "contact" && (
            <section className="contact-panel" id="contact"><p><BilingualText entry={content.ai.contactCopy} primary="zh" /></p><a href="mailto:sshiyuanz@outlook.com"><BilingualText entry={content.ai.contactLink} primary="en" /></a><small><BilingualText entry={content.ai.contactEmail} primary="en" /></small></section>
          )}
        </div>
      </section>
    </SceneShell>
