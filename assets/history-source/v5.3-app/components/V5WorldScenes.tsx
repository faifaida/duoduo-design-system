...
const seedStars: VisitorStar[] = [
  { id: "light-01", nickname: "M.", city: "Shanghai", message: "在这里停了一会儿。自由和扎根原来真的可以是同一个问题。", date: "2026.07", reply: "谢谢你看见了这条暗线。", x: 58, y: 38 },
  { id: "light-02", nickname: "Lina", city: "Colombo", message: "Your route feels less like a résumé and more like a living tide.", date: "2026.07", x: 73, y: 30 },
  { id: "light-03", nickname: "阿岛", city: "Luoyang", message: "希望有一天能在懒懒岛见到真正从远方回来的人。", date: "2026.07", x: 86, y: 6 },
  { id: "light-04", nickname: "N.", city: "Bali", message: "Keep the roots moving.", date: "2026.07", reply: "I will.", x: 67, y: 41 },
];
      ...
      <section ref={starFieldRef} onPointerMove={wakeStars} onClick={(event) => event.target === event.currentTarget && setOpenStar(null)} className="life-voyage-stage">
        ...
        <motion.article className="voyage-story-note paper-texture" key={node.year} initial={{ opacity: 0, x: -14 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <div className={`voyage-stage-layout ${node.photos.length === 1 ? "has-one-photo" : ""}`}>
            <figure className="voyage-stage-photo primary"><img src={node.photos[0]} alt={`${text(node.title)} stage`} /></figure>
            <dl>
        ...
        <aside className="guestbook-panel">
          ...
          <form onSubmit={submitMessage}>
            <textarea name="message" required minLength={20} maxLength={200} onChange={(event) => setMessageLength(event.target.value.length)} placeholder="Write something kind…&#10;写下你的想法或祝福…" />
            ...
            <div className="guestbook-meta">
              ...
              <input name="email" type="email" placeholder="Email (private) · 邮箱（不公开）" />
            </div>
